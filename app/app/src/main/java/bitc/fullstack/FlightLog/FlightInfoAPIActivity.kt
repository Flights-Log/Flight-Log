...
//            서버에서 받아온 데이터를 출력
            ...
            if (flightInfoList.isNullOrEmpty()) {
              ...
              for (item in flightInfoList) {
                ...
                Log.d(
                  "fullstack503",
                  "항공사명 : ${item.airlineKorean}, 항공편명 : ${item.domesticNum}, 출발 지역 : ${item.startcity}, 도착 지역 : ${item.arrivalcity}, 출발시간 : ${item.domesticStartTime}, 도착시간 : ${item.domesticArrivalTime} "
                )
    ...
    binding.btnInterFlightInfo.setOnClickListener {
      ...
      call.enqueue(object : Callback<InterFlightResponse> {
        ...
        override fun onResponse(
          call: Call<InterFlightResponse>,
          res: Response<InterFlightResponse>
        ) {
          if (res.isSuccessful) {
            Log.d("fullstack503", "응답 본문: ${res.body()?.toString()}")
            ...
            if (flightInfoList.isNullOrEmpty()) {
              ...
              for (item in flightInfoList) {
                ...
                Log.d(
                  "fullstack503",
                  "항공사명 : ${item.airlineKorean}, 항공편명 : ${item.internationalNum}, 출발 지역 : ${item.deptcity}, 도착 지역 : ${item.arrvcity}, 출발시간 : ${item.internationalTime} "
                )
